...
Mail de bienvenue envoyé au nouvel inscrit, déclenché par `login.html` juste après `signUp`.

| Champ | Valeur |
|-------|--------|
| **Auth** | Bearer token de la session (`verifyAuth`) — l'envoi se fait vers l'email du token, donc vers soi-même uniquement (pas de vecteur de spam) |
| **Body** | Aucun (l'email et le nom proviennent du token / `user_metadata`) |
| **Réponse** | `{ success: true, id }`, ou `{ success: true, skipped: 'already_sent' }`, ou `401` / `502` |

**Flow** :
1. `verifyAuth(req)` → user. Sinon `401`.
2. Garde-fou idempotence : si `user_metadata.welcome_email_sent` est vrai → skip (anti-doublon double-submit / reload).
3. Envoi via `lib/mailgun-send.js` (template HTML inline charté Léon, tutoiement, CTA « Ouvrir Léon »).
4. Sur succès, marque `user_metadata.welcome_email_sent = true` via `admin.auth.admin.updateUserById()` (non bloquant).

Côté front, l'appel est **best-effort** (`sendWelcomeEmail()` dans `login.html`, pas de `await`) : un échec n'interrompt jamais l'inscription.
