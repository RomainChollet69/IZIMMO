...
- **Correctif suivi (bas de carte sous le footer)** : la légende et les contrôles zoom Google passaient sous la bottom nav (`.m-nav`). 1re tentative (réduire `height` de `.main-container`) **inopérante** : la règle de base `.main-container { flex: 1 }` étire le conteneur via `flex-grow`, neutralisant tout `height` explicite. Vraie cause identifiée. Correctif retenu : réserver l'espace de la nav via `padding-bottom` sur le `body` (flex column, `box-sizing: border-box`, `100dvh`) → `.main-container` (flex:1) remplit l'espace au-dessus de la nav, sans dépendre de la hauteur du header.
...
- **Suppression de la légende prix/m²** : les ventes s'affichent en parcelles cyan uniformes (`renderSaleParcels`, `#00E5FF`), plus en pastilles colorées par prix. La légende `#legendDVF` (vert→violet par tranche) ne correspondait donc plus à rien et chevauchait le bouton filtres (FAB) sur mobile. Retrait du bloc HTML + de la ligne JS qui togglait son affichage. Légende DPE conservée (marqueurs DPE réellement colorés par classe).
- **Purge du code mort des marqueurs-ronds** : depuis le passage au modèle « parcelles cyan », les `markers` google.maps n'étaient plus jamais ajoutés à la carte (aucun `setMap(map)`, aucun `MarkerClusterer`). Tout l'habillage de ces ronds était mort et a été retiré (~176 lignes) : contenu InfoWindow par marqueur (single + multi-ventes), handlers `marker.addListener('click')`, `marker._parcelSales` (assigné, jamais lu), `getPriceColor()`. `renderMarkers()` ne crée plus que des marqueurs **position-only** (1 par parcelle), qui restent la source de positions de `renderSaleParcels()` et l'ancre InfoWindow de `showSaleDetail()` — comportement visible inchangé. `getPriceClass()` (badges prix/m² de la liste latérale) conservé, bien utilisé.
