...
**Filtre newsletters/rapports** (étape 4ter, `isPortalNewsletterOrReport`) : les domaines portails envoient aussi des newsletters et rapports de stats (ex : `news.leboncoin.fr`, « Rapport d'activité hebdomadaire » Bien'ici). Ils passent l'allowlist mais ne sont PAS des demandes de contact → écartés AVANT Claude si le domaine est un sous-domaine d'emailing (`news.*`/`newsletter.*`...) OU si le sujet matche un rapport/newsletter. Volontairement conservateur (zéro vraie demande bloquée). Le filtre Gmail généré (`downloadGmailFilter`, `PORTAL_FILTER_EXCLUDE`) ajoute la même exclusion côté agent.
